...
public class TargetHandler : MonoBehaviour {
    ...
    [SerializeField]
    private NavigationController navigationController; /// Instance were the class is initialized
    [SerializeField]
    private TMP_Dropdown targetDataDropdown1; /// dropdown menu filled with targets on first menu page
    [SerializeField]
    private TMP_Dropdown targetDataDropdown2; /// dropdown menu filled with targets on last menu page
    [SerializeField]
    private TMP_Dropdown targetDataDropdown3; /// dropdown menu filled with targets on adminpanel

    [SerializeField]
    private Slider navigationZOffset; /// Slider to change z location of selected target
    [SerializeField]
    private Slider navigationXOffset; /// Slider to change x location of selected target

    private string item; /// variable to store the current selected target
    private int value; /// variable to store the selected value of a dropdown menu

    [SerializeField]
    private GameObject targetObjectPrefab; /// Prefab to create targets from
    [SerializeField]
    private Transform[] targetObjectsParentTransforms; /// location were to store targets in environment

    private List<TargetFacade> currentTargetItems = new List<TargetFacade>(); /// List to store current targets

    Target[] targets; /// array to store current targets
    List<Target> targetlist; /// List to store current targets

    int selected = 0; /// show if a new item gets selected by user to show location by changing color
    private string saveprevious; /// save previous item to check if a new item was selected
    ...
    [SerializeField]
    public InputField NewTargetNameInput; /// Input field to give the new target added a name

    private string outputstring = ""; /// String for the JSON ulpoaded to server when save gets clicked
    public const string LoadURL = "https://ar-app-rest-api.herokuapp.com/positions"; /// Address for loading the data to create target gameobjects
    public const string SaveURL = "https://ar-app-rest-api.herokuapp.com/updatepositions"; /// Address for uploading the data when targets get updated in position etc.
    ...
    /// </summary>
    /// <returns> Returns the JSON as aTask<string> </returns>
    ...
    private async void GenerateTargetItems() {   
